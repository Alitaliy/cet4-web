...
description: 开展 CET-4 四级词汇训练、核心词义判分和易混词纠错；按要求生成并批量补充未收录的新词，写入与拾词网页共用的本地词库。用于继续训练、补充下一批新词、扩充词库、复习错词或查询进度。
---
...
用户说“补充下一批”“再加 20 个四级词”或要求扩充词库时，按 [references/ADD_WORDS.md](references/ADD_WORDS.md) 选词并执行 `add-batch`。数量未指定时默认补充 20 个；排除词库中所有已有词，包括已掌握和已暂停的词。只复习或查询进度时不自动扩词，除非用户已约定自动补词。

选词由当前 Codex 完成，脚本负责校验、去重和保存，不调用额外的 AI API。初始 48 词不是总量上限。新词保存为 `new`、零测试次数，不能把“添加成功”当成“已测试/已掌握”。只有用户要求立即训练时，才接着出题；否则报告实际新增数量，并提示在网页点击“重新加载”。
